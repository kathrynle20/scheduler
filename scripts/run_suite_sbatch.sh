...
#
# Submit with:
#   sbatch scripts/run_suite_sbatch.sh                      # 3 trials, all configs, GPUs/rate from YAML
#   sbatch scripts/run_suite_sbatch.sh 5                    # 5 trials, all configs
#   sbatch scripts/run_suite_sbatch.sh 3 ptq                # 3 trials, PTQ pair only
#   sbatch scripts/run_suite_sbatch.sh 3 "" 2 1.4           # explicit: 2 GPUs, 1.4 hz arrival rate
#
...
bash scripts/run_suite.sh "$N_TRIALS" "$FILTER" "$NUM_GPUS" "$ARRIVAL_RATE"
